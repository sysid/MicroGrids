...
def Load_results1(instance):
    ...
    if instance.Delta_Time() >= 1 and type(instance.Delta_Time()) == type(1.0):  # if the step is in hours and minutes
        ...
        columns = pd.date_range(start=instance.StartDate(), periods=instance.Periods(),
                                freq=(hour + 'h' + minutes + 'min'))
        ...
        columns = pd.date_range(start=instance.StartDate(),
                                periods=instance.Periods(),
                                freq=(
                                        str(instance.Delta_Time()) + 'h'))  # Creation of an index with a start date and a frequency
        ...
        columns = pd.date_range(start=instance.StartDate(),
                                periods=instance.Periods(),
                                freq=(str(int(
                                    instance.Delta_Time() * 60)) + 'min'))  # Creation of an index with a start date and a frequency

    Scenarios.columns = columns
    Scenarios = Scenarios.transpose()

    Scenarios.to_excel(
        'Results/Time_Series.xls')  # Creating an excel file with the values of the variables that are in function of the periods
...
def Load_results2(instance):
    ...
    Size_variables.to_excel(
        'Results/Size.xls')  # Creating an excel file with the values of the variables that does not depend of the periods
...
def Load_results1_binary(instance):
    ...
    if instance.Delta_Time() >= 1 and type(instance.Delta_Time()) == type(1.0):  # if the step is in hours and minutes
        ...
        columns = pd.date_range(start=instance.StartDate(),
                                periods=instance.Periods(),
                                freq=(
                                        hour + 'h' + minutes + 'min'))  # Creation of an index with a start date and a frequency
    elif instance.Delta_Time() >= 1 and type(instance.Delta_Time()) == type(1):  # if the step is in hours
        columns = pd.date_range(start=instance.StartDate(),
                                periods=instance.Periods(),
                                freq=(
...
def Plot_Energy_Total(instance, Time_Series, plot):
    ...
    if plot == 'No Average':
        ...
        for x in range(0,
                       instance.Periods()):  # Find the position form wich the plot will start in the Time_Series dataframe
            # foo = pd.DatetimeIndex(start=instance.PlotDay(),periods=1,freq='1h') # Asign the start date of the graphic to a dumb variable
            foo = pd.date_range(start=instance.PlotDay(), periods=1,
                                freq='1h')  # Asign the start date of the graphic to a dumb variable
        ...
        Plot_Data = Time_Series[Start_Plot:int(
            End_Plot)]  # Extract the data between the start and end position from the Time_Series
        # columns = pd.DatetimeIndex(start=instance.PlotDay(), periods=instance.PlotTime()*Periods_Day, freq=('1h'))
        columns = pd.date_range(start=instance.PlotDay(), periods=instance.PlotTime() * Periods_Day, freq=('1h'))
        ...
        ax5 = Plot_Data['Charge energy to the Battery'].plot(style='m',
                                                             linewidth=0.5)  # Plot the line of the energy flowing into the battery
        ...
        ax6 = Plot_Data['State_Of_Charge_Battery'].plot(style='k--', secondary_y=True, linewidth=2,
                                                        alpha=0.7)  # Plot the line of the State of charge of the battery
        ...
        State_Of_Charge_Battery = mlines.Line2D([], [], color='black', label='Estado de carga de la bateria',
                                                linestyle='--', alpha=0.7)
    ...
    State_Of_Charge_Battery = mlines.Line2D([], [], color='black', label='State_Of_Charge_Battery', linestyle='--',
                                            alpha=0.7)
    plt.legend(
        handles=[From_Generator, From_PV, From_Battery, To_Battery, Lost_Load, Energy_Demand, State_Of_Charge_Battery],
        bbox_to_anchor=(1.83, 1))
...
def Percentage_Of_Use(Time_Series):
    ...
    PercentageOfUse = {'Lost Load': 0, 'Energy PV': 0, 'Curtailment': 0, 'Energy Diesel': 0,
                       'Discharge energy from the Battery': 0, 'Charge energy to the Battery': 0}
...
def Energy_Flow(Time_Series):
    Energy_Flow = {'Energy_Demand': 0, 'Lost Load': 0, 'Energy PV': 0, 'Curtailment': 0, 'Energy Diesel': 0,
                   'Discharge energy from the Battery': 0, 'Charge energy to the Battery': 0}

    for v in Energy_Flow.keys():
        if v == 'Energy PV':
            Energy_Flow[v] = round((Time_Series[v].sum() - Time_Series['Curtailment'].sum() - Time_Series[
                'Charge energy to the Battery'].sum()) / 1000000, 2)
        else:
...
def Energy_Participation(Energy_Flow):
    ...
    c = {'Energy Diesel': 'Diesel Generator', 'Discharge energy from the Battery': 'Battery', 'Energy PV': 'From PV',
         'Lost Load': 'Lost Load'}
    labels = []
